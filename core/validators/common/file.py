...
class FileSizeValidator:
    ...
    def __eq__(self, other):
        return (
            isinstance(other, FileSizeValidator) and self.min_size == other.min_size and self.max_size == other.max_size
        )
...
class DocumentFileNameValidator(RegexValidator):
    ...
    def __init__(self, allowed_extensions, *, max_length: int = 255):
        ...
        normalized = {extension.lower().removeprefix(".") for extension in allowed_extensions}
        if not normalized:
            raise ValueError("allowed_extensions must not be empty")
