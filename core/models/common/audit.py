...
def _validate_performed_by(value: AbstractUser):
    if not isinstance(value, AbstractUser) or not getattr(value, "username", None):
        import sys

        caller = sys._getframe(1).f_code.co_name
        raise TypeError(
            f"{caller}() missing 1 required positional argument: 'performed_by' of type {AbstractUser.__name__}"
        )
    return value
...
class _AuditableQuerySet(models.QuerySet):
    ...
    def update(self, **kwargs):
        ...
        return super().update(updated_by=user.username, **kwargs)
